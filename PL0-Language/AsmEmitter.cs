    ...
    internal sealed class AsmEmitter
    {
        ...
        private static IEnumerable<string> Peephole(List<string> src)
        {
            ...
            for (int i = 0; i < src.Count; i++)
            {
                ...
                if (i + 1 < src.Count && cur.Trim() == "LIT 0" && src[i + 1].Trim() == "ADD") { i++; continue; }
                if (i + 1 < src.Count && cur.Trim() == "SWAP" && src[i + 1].Trim() == "SWAP") { i++; continue; }
                if (i + 1 < src.Count && cur.Trim() == "DUP" && src[i + 1].Trim() == "DROP") { i++; continue; }
                dst.Add(cur);
            }
            return dst;
        }
